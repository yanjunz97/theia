...
func startMigration(clickhouseMigrate *migrate.Migrate) error {
	theiaVersionNumber, err := getTheiaVerstionNumber()
	if err != nil {
...
func copyMigrators() error {
	bashCmd := fmt.Sprintf("mkdir -p %s", migratorPersistentPath)
	cmd := execCommand("bash", "-c", bashCmd)
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("error when creating folder: %s, error: %v", bashCmd, err)
	}
	bashCmd = fmt.Sprintf("cp -r %s/* %s", migratorTmpPath, migratorPersistentPath)
	cmd = execCommand("bash", "-c", bashCmd)
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("error when copying files: %s, error: %v", bashCmd, err)
	}
	return nil
}
...
func getTheiaVerstionNumber() (int, error) {
