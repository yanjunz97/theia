...
if [ ! -z "$THEIA_FROM_TAG" ]; then
    ...
    if [ $rc -ne 0 ]; then
        echoerr "$THEIA_FROM_TAG is not a valid Antrea tag"
        exit 1
    fi
...
TEHIA_VERSION_MAP="v0.1.0:v1.7.0, v0.2.0:v1.8.0"
ANTREA_FROM_TAG=$(echo $TEHIA_VERSION_MAP | tr ', ' '\n' | grep "$THEIA_FROM_TAG" | sed 's/:/ /g' | awk '{print $2}')
...
DOCKER_IMAGES=("k8s.gcr.io/e2e-test-images/agnhost:2.29" \
                ...
                "projects.registry.vmware.com/antrea/flow-aggregator:$ANTREA_FROM_TAG" \
...
function mutateConfigMap {
  sed -i -e "s|image: \"projects.registry.vmware.com/antrea/antrea-ubuntu:latest\"|image: \"antrea/antrea-ubuntu:latest\"|g" $ANTREA_YAML
  sed -i -e "s/#  FlowExporter: false/  FlowExporter: true/g" $ANTREA_YAML
  sed -i -e "s/flowPollInterval: \"5s\"/flowPollInterval: \"1s\"/g" $ANTREA_YAML
  sed -i -e "s/activeFlowExportTimeout: \"5s\"/activeFlowExportTimeout: \"2s\"/g" $ANTREA_YAML
  sed -i -e "s/idleFlowExportTimeout: \"15s\"/idleFlowExportTimeout: \"1s\"/g" $ANTREA_YAML

  perl -i -p0e 's/      # Enable is the switch to enable exporting flow records to ClickHouse.\n      enable: false/      # Enable is the switch to enable exporting flow records to ClickHouse.\n      enable: true/' $FLOW_AGGREGATOR_YAML
  sed -i -e "s|image: projects.registry.vmware.com/antrea/flow-aggregator:latest|image: antrea/flow-aggregator:latest|g" $FLOW_AGGREGATOR_YAML
  sed -i -e "s/    activeFlowRecordTimeout: 60s/    activeFlowRecordTimeout: 3500ms/g" $FLOW_AGGREGATOR_YAML
  sed -i -e "s/    inactiveFlowRecordTimeout: 90s/    inactiveFlowRecordTimeout: 6s/g" $FLOW_AGGREGATOR_YAML
  sed -i -e "s/      podLabels: false/      podLabels: true/g" $FLOW_AGGREGATOR_YAML
  sed -i -e "s/      commitInterval: \"8s\"/      commitInterval: \"1s\"/g" $FLOW_AGGREGATOR_YAML
  # These replacements are required to be compatible with Flow Aggregator before version v1.8.0
  perl -i -p0e 's/      # Enable is the switch to enable exporting flow records to ClickHouse.\n      #enable: false/      # Enable is the switch to enable exporting flow records to ClickHouse.\n      enable: true/' $FLOW_AGGREGATOR_YAML
  sed -i -e "s/    #activeFlowRecordTimeout: 60s/    activeFlowRecordTimeout: 3500ms/g" $FLOW_AGGREGATOR_YAML
  sed -i -e "s/    #inactiveFlowRecordTimeout: 90s/    inactiveFlowRecordTimeout: 6s/g" $FLOW_AGGREGATOR_YAML
  sed -i -e "s/      #podLabels: false/      podLabels: true/g" $FLOW_AGGREGATOR_YAML
...
ANTREA_YAML=$TMP_DIR/antrea.yml
FLOW_AGGREGATOR_YAML=$TMP_DIR/flow-aggregator.yml
mutateConfigMap

docker exec -i kind-control-plane dd of=/root/antrea-new.yml < $ANTREA_YAML
docker exec -i kind-control-plane dd of=/root/flow-aggregator-new.yml < $FLOW_AGGREGATOR_YAML
$ROOT_DIR/hack/generate-manifest.sh --local /data/clickhouse | docker exec -i kind-control-plane dd of=/root/flow-visibility-new.yml
...
TMP_ANTREA_DIR=$(mktemp -d $(dirname $0)/tmp.XXXXXXXX)
git clone --branch $ANTREA_FROM_TAG --depth 1 https://github.com/antrea-io/antrea.git $TMP_ANTREA_DIR
...
./hack/generate-manifest.sh --mode release > ./antrea.yml
ANTREA_YAML=./antrea.yml

export IMG_NAME=projects.registry.vmware.com/antrea/flow-aggregator
./hack/generate-manifest-flow-aggregator.sh --mode release > ./flow-aggregator.yml
FLOW_AGGREGATOR_YAML=./flow-aggregator.yml

mutateConfigMap

docker exec -i kind-control-plane dd of=/root/antrea.yml < $ANTREA_YAML
docker exec -i kind-control-plane dd of=/root/flow-aggregator.yml < $FLOW_AGGREGATOR_YAML
popd
rm -rf $TMP_ANTREA_DIR
...
# TODO: Before v0.2.0, we do not support --local option when generating manifest,
# so we need to copy the current script. We can skip this step after we update the release v0.1.0 scripts.
cp $ROOT_DIR/hack/generate-manifest.sh hack/generate-manifest.sh
./hack/generate-manifest.sh --mode release --local /data/clickhouse | docker exec -i kind-control-plane dd of=/root/flow-visibility.yml

popd
rm -rf $TMP_THEIA_DIR

rc=0
go test -v -run=TestUpgrade antrea.io/theia/test/e2e -provider=kind --logs-export-dir=$ANTREA_LOG_DIR --upgrade.toVersion=$CURRENT_VERSION || rc=$?
